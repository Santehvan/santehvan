...
export async function createUrlProduct({
  id,
  name,
  isAvailable,
  quantity,
...
export async function addParamsToProduct({ productId, params }: GetParams) {
  try {
    ...
    await product.params.push({
      name: "Товар",
      value: params.Model.replace(/ /g, "_"),
    });
...
export async function setDiscount({
  ...
  try {
    await connectToDB(); // Очікуємо на підключення до БД

    const products = await Product.find({ category });

    // Використовуйте `Promise.all` для того, щоб дочекатися всіх `save()`
    await Promise.all(
      products.map(async (product) => {
        product.priceToShow = product.price - (product.price / 100) * discount;
        await product.save();
      })
    );
    ...
    throw new Error(`Error applying discount to products: ${error.message}`);
  }
}

export async function ChangePriceValue({
  category,
  newPrice,
}: {
  category: string;
  ...
  try {
    ...
    await Promise.all(
      products.map(async (product) => {
        product.price = product.price + (product.price / 100) * newPrice;
