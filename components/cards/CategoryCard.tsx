...
const CategoryCard = ({label, pageLink, imageSrc}: Params) => {
  return (
    <article className="border  w-full h-80 rounded bg-gray-150 relative">
        ...
        <TransitionLink href={pageLink} className="w-[80%] mx-auto hover:bg-black border border-black hover:text-white block mt-7 py-4 text-center rounded-md transition-colors">{label}</TransitionLink>
    </article>
  )
}
